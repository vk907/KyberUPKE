...
class Kyber:
    ...
    def setrho(self):
        d = self.random_bytes(32)
        self.rho, _ = self._g(d)
    def upKey(self,pk,sk,sigma_old):
        # Unpack the public key
        t_hat_bytes, rho = pk[:-32], pk[-32:]
        ...
        sk_hat = self.M.decode_vector(sk_pke, self.k, 12, is_ntt=True)
        
        if sigma_old!=0:
            N=0
            r, N = self._generate_error_vector(sigma_old, self.eta_1, N)
            r_hat = r.to_ntt()

            # Generate the error vector e ∈ R^k
            e1, N = self._generate_error_vector(sigma_old, self.eta_1, N)
            e1_hat = e1.to_ntt()
            t_hatold = (A_hat @ r_hat) + e1_hat
            # Construct the public key
            t_hat1 = (A_hat @ s_hat) + e_hat
            t_hat=t_hat1+t_hat-t_hatold

            # Reduce vectors mod^+ q

            sk_new=sk_hat+s_hat-r_hat
        else:
                    # Construct the public key
            t_hat1 = (A_hat @ s_hat) + e_hat
            t_hat=t_hat1+t_hat
        ...
        t_hat.reduce_coefficients()
        sk_new.reduce_coefficients()
        
        
        pk = t_hat.encode(12) + rho
        _sk=sk_new.encode(12)
